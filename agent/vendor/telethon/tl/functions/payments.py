...
class GetResaleStarGiftsRequest(TLRequest):
    ...
    def _bytes(self):
        return b''.join((
            b'6\xa2_z',
            struct.pack('<I', (0 if self.sort_by_price is None or self.sort_by_price is False else 2) | (0 if self.sort_by_num is None or self.sort_by_num is False else 4) | (0 if self.for_craft is None or self.for_craft is False else 16) | (0 if self.attributes_hash is None or self.attributes_hash is False else 1) | (0 if self.attributes is None or self.attributes is False else 8)),
    ...
    def from_reader(cls, reader):
        ...
        _offset = reader.tgread_string()
        _limit = reader.read_int()
        return cls(gift_id=_gift_id, offset=_offset, limit=_limit, sort_by_price=_sort_by_price, sort_by_num=_sort_by_num, for_craft=_for_craft, attributes_hash=_attributes_hash, attributes=_attributes)
