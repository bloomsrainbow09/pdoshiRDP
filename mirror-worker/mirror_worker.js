...
async function main() {
  ...
  while (true) {
    try {
      ...
      const N = Math.max(1, Number(process.env.MIRROR_PARALLEL || cfg.mirror_parallel || 15));
      if (!PAT || !USER) { console.log('missing mirror_pat/mirror_user'); await sleep(30000); continue; }
      await pg.query(INTAKE);
      ...
      if (jobs.length) { console.log('syncing ' + jobs.length + ' (parallel ' + N + ') ...'); await pool(jobs, N, j => mirrorOne(pg, PAT, USER, j)); }
